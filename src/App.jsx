...
const emptyH={empresa:"",fecha:"",proyecto:"",sondaje:"",maquina:"",ubicacion:"",
  inclinacion:"90",azimut:"",turno:"Día",constante:"",broca:"",brocaSerie:"",
  ...
  observaciones:"",dt:Array(20).fill("")};
...
export default function App(){
  ...
  return(
    <div style={root}>
      ...
      {/* ── TABS ── */}
      <div style={tabBar}>
        {[["calc","⛏️","Campo"],["saved","📂","Guardados"],["report","📋","Reporte"]].map(([id,ic,lb])=>(
        ...
        <div style={{background:t.card,borderBottom:`1px solid ${t.border}`,padding:"12px"}}>
          <div style={hGrid}>
            ...
            {[["sondaje","Sondaje","506-62",1],["maquina","N° Máquina","ED20-08",1],
              ...
              ["fecha","Fecha","",1,"date"],["turno","Turno","",1,"sel"],
              ["mtsProgram","Mts Program.","",1],["constante","Constante (m)","1.60",1],
              ["inclinacion","Inclinación°","90",1],["azimut","Azimut°","000",1],
              ["broca","Broca","HA-111492",1],["rshell","R. Shell","61-7205",1],
              ["zapata","Zapata","553185",1],
      ...
      {tab==="report"&&<div>
        ...
        <div style={{padding:"10px",background:t.bg}} id="report-content">
          ...
          {/* Encabezado */}
          <div style={{border:`1px solid ${t.border}`,borderRadius:8,marginBottom:6,overflow:"hidden",background:t.card}}>
            <div style={{display:"flex",alignItems:"stretch",borderBottom:`1px solid ${t.border}`}}>
              <div style={{background:t.card2,padding:"10px 12px",display:"flex",alignItems:"center",
                gap:8,minWidth:150,borderRight:`1px solid ${t.border}`}}>
                <span style={{fontSize:22}}>⛏️</span>
                <div>
                  <div style={{fontSize:13,fontWeight:800,color:t.text}}>{h.empresa||"EMPRESA"}</div>
                  <div style={{fontSize:8,color:t.text3,textTransform:"uppercase",letterSpacing:1}}>SIG - OPERACIONES</div>
                </div>
              </div>
              <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",
                padding:"8px",borderRight:`1px solid ${t.border}`}}>
                <div style={{fontSize:12,fontWeight:800,color:t.blue,textAlign:"center",textTransform:"uppercase"}}>
                  REPORTE DIARIO DE PERFORACIÓN DIAMANTINA SUPERFICIE</div>
              </div>
              <div style={{background:t.card2,padding:"6px 10px",minWidth:80,display:"flex",
                flexDirection:"column",justifyContent:"center"}}>
                {["SIG-OP-RE-PO-007","Rev: 01","2026"].map(s=>(
                  <div key={s} style={{fontSize:8,color:t.text3,textAlign:"right"}}>{s}</div>
                ))}
              </div>
            </div>
            {/* Datos */}
            {[[["FECHA",h.fecha||"___/___/____"],["N° MÁQUINA",h.maquina],["N° SONDAJE",h.sondaje],["AZIMUT",h.azimut],["INCLINACIÓN",h.inclinacion?h.inclinacion+"°":""]],
              [["PROYECTO",h.proyecto,2],["UBICACIÓN SONDAJE",h.ubicacion,2],["MTS. PROGRAMADOS",h.mtsProgram]],
              [["TURNO",h.turno]]
            ].map((rowFields,ri)=>(
              <div key={ri} style={{display:"flex",borderBottom:ri<2?`1px solid ${t.border}`:"none"}}>
                {rowFields.map(([k,v,sp])=>(
                  <div key={k} style={{flex:sp||1,padding:"4px 8px",borderRight:`1px solid ${t.border}`,
                    display:"flex",flexDirection:"column"}}>
                    <span style={{fontSize:8,color:t.text3,fontWeight:700,textTransform:"uppercase"}}>{k}</span>
          ...
          {/* Control Avances + Distribución */}
          <div style={{display:"flex",gap:6,marginBottom:6}}>
            <div style={{flex:1}}>
              <div style={secH}>CONTROL DE AVANCES</div>
              <table style={{borderCollapse:"collapse",width:"100%",background:t.card}}>
                <thead><tr style={{background:t.th}}>
                  {["DESCRIPCIÓN","Ø","CANTIDAD"].map(h2=>(
                    <th key={h2} style={{...th,fontSize:8,padding:"4px"}}>{h2}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {[["1. Prof. al inicio del turno:","HQ","0.00 m"],
                    ["2. Prof. al final del turno:","HQ",totP>0?totP.toFixed(2)+" m":""],
                    ["3. Sobrante al final del turno:","HQ",comp[comp.length-1]?.sob!=null?comp[comp.length-1].sob.toFixed(2)+" m":""],
                    ["4. Long. de tubería al final:","HQ",h.constante?h.constante+" m":""],
                    ["5. Metros perforados:","HQ",totP.toFixed(2)+" m"],
                    ["6. Muestra recuperada:","HQ",totR.toFixed(2)+" m"],
                  ].map(([d,o,v],i)=>(
                    <tr key={i} style={{background:i%2===0?t.row1:t.row2}}>
                      <td style={{...td,fontSize:10,textAlign:"left",paddingLeft:6,color:t.text}}>{d}</td>
                      <td style={{...td,fontSize:10,fontWeight:700,color:t.blue,width:30}}>{o}</td>
                      <td style={{...td,fontSize:10,fontWeight:700,color:t.text,width:60}}>{v}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{flex:1}}>
              <div style={secH}>DISTRIBUCIÓN DE TIEMPO</div>
              <table style={{borderCollapse:"collapse",width:"100%",background:t.card}}>
                <thead><tr style={{background:t.th}}>
                  {["#","DESCRIPCIÓN","HORAS"].map(h2=>(
                    <th key={h2} style={{...th,fontSize:8,padding:"4px"}}>{h2}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {DT.map((d,i)=>(
                    <tr key={i} style={{background:i%2===0?t.row1:t.row2}}>
                      <td style={{...td,fontSize:9,width:16,color:t.text3}}>{i+1}</td>
                      <td style={{...td,fontSize:9,textAlign:"left",paddingLeft:4,color:t.text}}>{d}</td>
                      <td style={{...td,fontSize:9,color:t.text}}>{h.dt[i]||""}</td>
                    </tr>
                  ))}
                  <tr style={{background:t.th}}>
                    <td colSpan={2} style={{...td,fontWeight:800,fontSize:9,color:t.thTxt}}>TOTAL HORAS DE TURNO</td>
                    <td style={{...td,fontWeight:800,color:t.yellow}}>{hTrab?hTrab+" h":""}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          {/* Datos Control */}
          <div style={secH}>DATOS DE CONTROL</div>
          <table style={{borderCollapse:"collapse",width:"100%",background:t.card,marginBottom:6}}>
            <tbody>
              {[["1. Horómetro inicial de máquina diamantina:",h.horometroIni?h.horometroIni+" h":""],
                ["2. Consumo de combustible de máquina turno:",h.combustible?h.combustible+" gln":""],
                ["3. Consumo de agua en la perforación turno:",h.agua?h.agua+" gln":""],
                ["4. Horómetro final de máquina diamantina:",h.horometroFin?h.horometroFin+" h":""],
              ].map(([d,v],i)=>(
                <tr key={i} style={{background:i%2===0?t.row1:t.row2}}>
                  <td style={{...td,fontSize:10,textAlign:"left",paddingLeft:6,color:t.text}}>{d}</td>
                  <td style={{...td,fontSize:10,fontWeight:700,color:t.text,width:80}}>{v}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {/* Aditivos */}
          <div style={secH}>ADITIVOS</div>
          <table style={{borderCollapse:"collapse",width:"100%",background:t.card,marginBottom:6}}>
            <thead><tr style={{background:t.th}}>
              {["ADITIVOS","CANTIDAD"].map(h2=><th key={h2} style={{...th,fontSize:8,padding:"4px"}}>{h2}</th>)}
            </tr></thead>
            <tbody>
              {[[h.aditivo1,h.aditivo1Cant],[h.aditivo2,h.aditivo2Cant],["",""]].map(([a,c2],i)=>(
                <tr key={i} style={{background:i%2===0?t.row1:t.row2}}>
                  <td style={{...td,fontSize:10,textAlign:"left",paddingLeft:6,color:t.text}}>{a}</td>
                  <td style={{...td,fontSize:10,color:t.text}}>{c2}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {/* Accesorios */}
          <div style={secH}>CONTROL DE RECORD ACCESORIOS DE PERFORACIÓN</div>
          <table style={{borderCollapse:"collapse",width:"100%",background:t.card,marginBottom:6}}>
            <thead><tr style={{background:t.th}}>
              {["ACCESORIO","Ø","MARCA","SERIE","CÓDIGO","DESDE","HASTA","RECORD ACUM."].map(h2=>(
                <th key={h2} style={{...th,fontSize:8,padding:"3px"}}>{h2}</th>
              ))}
            </tr></thead>
            <tbody>
              {[["BROCA","HQ","HA9",h.brocaSerie,h.brocaCodigo,h.brocaDesde,h.brocaHasta,
                  h.brocaDesde&&h.brocaHasta?(parseFloat(h.brocaHasta)-parseFloat(h.brocaDesde)).toFixed(2):""],
                ["R. SHELL","HQ","",h.rshellSerie,h.rshellCodigo,h.rshellDesde,h.rshellHasta,
                  h.rshellDesde&&h.rshellHasta?(parseFloat(h.rshellHasta)-parseFloat(h.rshellDesde)).toFixed(2):""],
                ["CASING SHOE","","","","","","",""],
              ].map((row2,i)=>(
                <tr key={i} style={{background:i%2===0?t.row1:t.row2}}>
                  {row2.map((v,j)=><td key={j} style={{...td,fontSize:10,color:t.text}}>{v}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {/* Perforación */}
          <div style={secH}>PERFORACIÓN (m)</div>
          <table style={{borderCollapse:"collapse",width:"100%",background:t.card,marginBottom:6}}>
            <thead><tr style={{background:t.th}}>
            ...
            <tbody>
              {vRows.map(({r,c,i},n)=>{
                const desde=i===0?0:(comp[i-1]?.prof||0);
                const pCol2=!c.pct?t.text3:+c.pct>=90?t.green:+c.pct>=70?t.yellow:t.red;
                return(
                  <tr key={i} style={{background:n%2===0?t.row1:t.row2}}>
                    <td style={{...td,fontSize:10,color:t.text}}>{n+1}</td>
                    <td style={{...td,fontSize:10,color:t.text}}>{desde.toFixed(2)}</td>
                    <td style={{...td,fontSize:10,color:t.text}}>{c.prof.toFixed(2)}</td>
                    <td style={{...td,fontSize:10,color:t.text}}>{c.perf.toFixed(2)}</td>
                    <td style={{...td,fontSize:10,color:t.text}}>{r.recuperado||""}</td>
                    <td style={{...td,fontSize:10,color:pCol2,fontWeight:700}}>{c.pct?c.pct+"%":""}</td>
                    <td style={{...td,fontSize:10,color:t.text}}>{r.recFluido||""}</td>
                    <td style={{...td,fontSize:10,color:t.text}}>{r.terreno||""}</td>
                  </tr>
                );
              })}
              {Array(Math.max(0,8-vRows.length)).fill(null).map((_,i)=>(
                <tr key={"e"+i} style={{background:i%2===0?t.row1:t.row2}}>
                  {Array(8).fill(null).map((_,j)=><td key={j} style={{...td,fontSize:10}}>&nbsp;</td>)}
                </tr>
              ))}
              <tr style={{background:t.th}}>
                <td colSpan={3} style={{...td,fontWeight:800,color:t.thTxt,textAlign:"center"}}>TOTAL</td>
                <td style={{...td,fontWeight:800,color:t.green}}>{totP.toFixed(2)}</td>
                <td style={{...td,fontWeight:800,color:t.blue}}>{totR.toFixed(2)}</td>
                <td style={{...td,fontWeight:800,color:!pctG?t.text3:+pctG>=90?t.green:+pctG>=70?t.yellow:t.red}}>
                  {pctG?pctG+"%":"—"}</td>
          ...
          {/* Observaciones */}
          <div style={secH}>OBSERVACIONES IMPORTANTES:</div>
          <div style={{border:`1px solid ${t.border}`,background:t.card,padding:"8px",
            minHeight:50,fontSize:10,color:t.text,marginBottom:6}}>
            {h.observaciones||"\u00a0"}
          </div>

          {/* Firmas */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginTop:16}}>
            {["PERSONAL DE TURNO",h.empresa||"EMPRESA","CLIENTE"].map(lb=>(
              <div key={lb} style={{textAlign:"center",padding:"8px",background:t.card,
                border:`1px solid ${t.border}`,borderRadius:6}}>
                <div style={{borderBottom:`1px solid ${t.border}`,marginBottom:6,height:28}}/>
                <div style={{fontSize:9,color:t.text3,textTransform:"uppercase",fontWeight:700}}>{lb}</div>
              </div>
            ))}
          </div>
